...
trait SparkMLBase extends OffLogger {
  ...
  def read_bucket(input_paths: PathParameter[_]): DataFrame = {
      ...
      if (mode.check_num_buckets) {
        ...
        assert(
          ...
          f"Number of specified buckets [${mode.num_buckets}] doesn't match with number of "
            + f"discovered buckets [${numDiscoveredBuckets}]."
        )
      }
     ...
     */
    def write_output_local(output_df: MLWritable): Unit = {
      output_df
    }

    def write_output_yarn(output_df: MLWritable): Unit = {
      val path = output_path.getValue
      output_df.write.save(path)
    }
